    ...
    <div data-print-target class="grid gap-4 {{ ($size ?? 'small') === 'large' ? 'grid-cols-1' : 'grid-cols-2' }}">
        @foreach ($registries as $item)
            ...
            <div class="dmims-barcode-item">
                @include('filament.barcode-label', ['barcode' => $registry->barcode, 'type' => $registry->barcode_type, 'size' => $size ?? 'small', 'title' => $itemTitle, 'standalone' => false, 'copies' => $copies ?? 1, 'showName' => $showName ?? true, 'showBarcodeText' => $showBarcodeText ?? true])
            </div>
        @endforeach
    </div>
</div>
